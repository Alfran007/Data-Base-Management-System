   ...
   <button onclick="myFunction()" class="dropbtn" style="width:180%;margin-left:-20%;">WELCOME <?php echo $_SESSION['adminid'] ?>  &nbsp; <i class="fa fa-user"></i></button>
    ...
    Sign out</a>
           ...
           <form id="signup" data-parsley-validate="" class="ng-pristine ng-valid" novalidate="" action="" method="post">
                 <div class="login-block" style="height:25%;">


                     <h1>Add Route</h1>
                     <input type="number" value="" placeholder="Enter Route Number" name="routeno" required="">
                     <input id="signup-username" type="text" name="source" data-parsley-required="true" data-parsley-trigger="change" required="" placeholder="Source">
                     <input id="signup-username" type="text" name="destination" data-parsley-required="true" data-parsley-trigger="change" required="" placeholder="Destination">


  				  <input name="action" type="submit" value="Add Route" style="position: relative;" onclick="Signup()">
                    <br>
           ...
           if(isset($_POST['action']))
           {
             if($_POST['action']=="Add Route"){
               ...
               $searchq=mysqli_query($connection,"SELECT * FROM `route` WHERE routeno = '".$routeno."'");
               $results = mysqli_fetch_array($searchq);
               if(count($results)>=1)
               {
               echo "<br>";
               echo "<div id='success1'  style='color:green;text-align:center; font-weight:bold;border: 1px solid green; height:8%; width:35%; padding:1%; background-color:lightgreen;margin-left:33%;'>"."Route Number Already exist"."</div>";
               ob_flush();
               flush();
               sleep(4);
               echo "<script>document.getElementById('success1').style.display = 'none'</script>";
                 ...
                 mysqli_query($connection,"INSERT INTO `route` (`routeno`, `source`, `destination`) VALUES ('".$routeno."','".$source."','".$destination."')");
                 echo "<br>";
                 echo "<div id='success'  style='color:green;text-align:center; font-weight:bold;border: 1px solid green; height:8%; width:35%; padding:1%; background-color:lightgreen;margin-left:33%;'>"."Route Successfully Added"."</div>";
                 ob_flush();
                 flush();
                 sleep(4);
